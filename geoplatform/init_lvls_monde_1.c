...
*/
   ...
   /* initialise le niveau 1 du monde 1 */
void lvl_1_1 (Var *var, Gal *gal) {
   ...
   /* initialise le niveau 2 du monde 1 */
void lvl_2_1 (Var *var, Gal *gal) {


   int i = 0; //  pour parcourir les boucles for
   int l = 1; // pour mois de fatigue pour moi
   ...
   /* initialise le niveau 3 du monde 1 */
void lvl_3_1 (Var *var, Gal *gal) {
   ...
   for (i = 0 ; i < NB_SOLSEC ; i += 1) {
      ...
      var -> p_lvl [l].sol_sec [i].w = 0;
         ...
         /* le 1er */
   var -> p_lvl [l].pics [0].Rect.x = LONG_WIND + 350; // on place le premier pic en x
   var -> p_lvl [l].pics [0].Rect.y = HAUT_WIND - 75 - HAUT_PICS_NORM * 1.5; // on place le premier pic en ordonnées
   var -> p_lvl [l].pics [0].Rect.h = HAUT_PICS_NORM * 1.5; // on redimensionne la hauteur pic
   var -> p_lvl [l].pics [0].Rect.w = HAUT_PICS_NORM * 1.5 / CORR_PIC_H_W; // on redimensionne la longueur premier pic
   placer_rects_pic (&var -> p_lvl [l].pics [0]); // on place correctement chacun des rects du pic

         /* le 1er */
   var -> p_lvl [l].pics [1].Rect.x = LONG_WIND + 500; // on place le premier pic en x
   var -> p_lvl [l].pics [1].Rect.y = 50; // on place le premier pic en ordonnées
   var -> p_lvl [l].pics [1].Rect.h = HAUT_PICS_NORM * 1.5; // on redimensionne la hauteur pic
   var -> p_lvl [l].pics [1].Rect.w = HAUT_PICS_NORM * 1.5 / CORR_PIC_H_W; // on redimensionne la longueur premier pic
   placer_rects_pic_returned (&var -> p_lvl [l].pics [1]); // on place correctement chacun des rects du pic
   var -> p_lvl [l].pics [1].text_pic = gal -> pic_returned; // on redimensionne la longueur pic
   ...
   var -> p_lvl [l].dead_zones [0].dead.x = LONG_WIND + 55; // zone de mort qui correspond au début de la zone de vol
   var -> p_lvl [l].dead_zones [0].dead.y = 0; // zone de mort qui correspond au début début de la zone de vol
   var -> p_lvl [l].dead_zones [0].dead.h = 50; // zone de mort qui correspond au début début de la zone de vol
   var -> p_lvl [l].dead_zones [0].dead.w = 1000; // zone de mort qui correspond au début début de la zone de vol
      /* le couleur de la zone de mort */
   var -> p_lvl [l].dead_zones [0].col.r = 2; // composente rouge
   var -> p_lvl [l].dead_zones [0].col.g = 230; // composente verte
   var -> p_lvl [l].dead_zones [0].col.b = 230; // composente bleue

   var -> p_lvl [l].dead_zones [1].dead.x = LONG_WIND + 55; // zone de mort qui correspond au début de la zone de vol
   var -> p_lvl [l].dead_zones [1].dead.y = HAUT_WIND - 75; // zone de mort qui correspond au début du sol secondaire #1, ordonnées
   var -> p_lvl [l].dead_zones [1].dead.h = 75; // zone de mort qui correspond au début du sol secondaire #1, hauteur
   var -> p_lvl [l].dead_zones [1].dead.w = 1000; // zone de mort qui correspond au début du sol secondaire #1, longueur
   ...
   var -> p_lvl [l].dead_zones [2].dead.x = LONG_WIND + 675; // zone de mort qui correspond au début de la zone de vol
   var -> p_lvl [l].dead_zones [2].dead.y = HAUT_WIND - 275; // zone de mort qui correspond au début du sol secondaire #1, ordonnées
   var -> p_lvl [l].dead_zones [2].dead.h = 75; // zone de mort qui correspond au début du sol secondaire #1, hauteur
   var -> p_lvl [l].dead_zones [2].dead.w = 100; // zone de mort qui correspond au début du sol secondaire #1, longueur
   ...
   for (i = 2 ; i < NB_CANNONS ; i += 1) {
      ...
      /* on positionne l'arrivée */
   var -> arrivee.rect.x = LONG_WIND + 2000; // on positionne le sprite arrivee en x
   var -> arrivee.rect.h = 125; // on lui donne sa hauteur en h
   var -> arrivee.rect.y = var -> p_lvl [l].Sol.y - 15 - var -> arrivee.rect.h; // on positionne le sprite arrivee en y
   var -> arrivee.rect.w = 130 / (180 / 125); // on lui donne sa longueur correspondante au h sans la déformer (en réalité, l'image fait 180 par 130p)
}
   ...
   /* initialise le niveau 4 du monde 1 */
void lvl_4_1 (Var *var, Gal *gal) {
...
///////////////////////////////
   ...
   var -> p_lvl [l].sol_sec [2].x = LONG_WIND + 2090; // sol secondaire, abscices
   var -> p_lvl [l].sol_sec [2].y = var -> p_lvl [l].Sol.y - 25; // sol secondaire, ordonnées
   var -> p_lvl [l].sol_sec [2].h = 25; // sol secondaire, hauteur
   var -> p_lvl [l].sol_sec [2].w = 40; // sol secondaire, longueur

   var -> p_lvl [l].sol_sec [3].x = LONG_WIND + 2150; // sol secondaire, abscices
   var -> p_lvl [l].sol_sec [3].y = var -> p_lvl [l].Sol.y - 100; // sol secondaire, ordonnées
   var -> p_lvl [l].sol_sec [3].h = 100; // sol secondaire, hauteur
   var -> p_lvl [l].sol_sec [3].w = 40; // sol secondaire, longueur

   var -> p_lvl [l].sol_sec [4].x = LONG_WIND + 2320; // sol secondaire, abscices
   var -> p_lvl [l].sol_sec [4].y = var -> p_lvl [l].Sol.y - 100; // sol secondaire, ordonnées
   var -> p_lvl [l].sol_sec [4].h = 100; // sol secondaire, hauteur
   var -> p_lvl [l].sol_sec [4].w = 40; // sol secondaire, longueur

   var -> p_lvl [l].sol_sec [5].x = LONG_WIND + 2510; // sol secondaire, abscices
   var -> p_lvl [l].sol_sec [5].y = var -> p_lvl [l].Sol.y - 50; // sol secondaire, ordonnées
   var -> p_lvl [l].sol_sec [5].h = 50; // sol secondaire, hauteur
   var -> p_lvl [l].sol_sec [5].w = 30; // sol secondaire, longueur

/////////////////////////////////////////////

   var -> p_lvl [l].sol_sec [6].x = LONG_WIND + 3000; // sol secondaire, abscices
   var -> p_lvl [l].sol_sec [6].y = var -> p_lvl [l].Sol.y - 30; // sol secondaire, ordonnées
   var -> p_lvl [l].sol_sec [6].h = 30; // sol secondaire, hauteur
   var -> p_lvl [l].sol_sec [6].w = 3000; // sol secondaire, longueur

   var -> p_lvl [l].dead_zones [1].dead.x = LONG_WIND + 3002; // zone de mort qui correspond au début de la zone de vol
   var -> p_lvl [l].dead_zones [1].dead.y = var -> p_lvl [l].sol_sec [6].y + 2; // zone de mort qui correspond au début du sol secondaire #1, ordonnées
   ...
   var -> p_lvl [l].sol_sec [7].x = LONG_WIND + 3200; // sol secondaire, abscices
   var -> p_lvl [l].sol_sec [7].y = var -> p_lvl [l].Sol.y - 60; // sol secondaire, ordonnées
   var -> p_lvl [l].sol_sec [7].h = 60; // sol secondaire, hauteur
   var -> p_lvl [l].sol_sec [7].w = 2850; // sol secondaire, longueur

   var -> p_lvl [l].dead_zones [2].dead.x = LONG_WIND + 3202; // zone de mort qui correspond au début de la zone de vol
   var -> p_lvl [l].dead_zones [2].dead.y = var -> p_lvl [l].sol_sec [7].y + 2; // zone de mort qui correspond au début du sol secondaire #1, ordonnées
   ...
   var -> p_lvl [l].dead_zones [3].dead.y = var -> p_lvl [l].sol_sec [7].y; // zone de mort qui correspond au début début de la zone de vol
   var -> p_lvl [l].dead_zones [3].dead.h = var -> p_lvl [l].sol_sec [7].h; // zone de mort qui correspond au début début de la zone de vol
   ...
   var -> p_lvl [l].sol_sec [8].y = 0; // sol secondaire, ordonnées
   ...
   var -> p_lvl [l].dead_zones [4].dead.h = var -> p_lvl [l].sol_sec [8].h; // zone de mort qui correspond au début du sol secondaire #1, hauteur
...
///////////////////////////
   ...
   var -> p_lvl [l].pics [4].Rect.x = LONG_WIND + 2090; // on place le premier pic en x
   var -> p_lvl [l].pics [4].Rect.y = var -> p_lvl [l].sol_sec [2].y - HAUT_PICS_NORM; // on place le premier pic en ordonnées
   ...
   var -> p_lvl [l].pics [5].Rect.x = LONG_WIND + 2210; // on place le premier pic en x
   var -> p_lvl [l].pics [5].Rect.y = var -> p_lvl [l].Sol.y - 140; // on place le premier pic en ordonnées
   var -> p_lvl [l].pics [5].Rect.h = 140; // on redimensionne la hauteur pic
   var -> p_lvl [l].pics [5].Rect.w = 120 / CORR_PIC_H_W; // on redimensionne la longueur premier pic
   placer_rects_pic (&var -> p_lvl [l].pics [5]); // on place correctement chacun des rects du pic

   var -> p_lvl [l].pics [6].Rect.x = LONG_WIND + 2400; // on place le premier pic en x
   var -> p_lvl [l].pics [6].Rect.y = var -> p_lvl [l].Sol.y - 65; // on place le premier pic en ordonnées
   var -> p_lvl [l].pics [6].Rect.h = 65; // on redimensionne la hauteur pic
   var -> p_lvl [l].pics [6].Rect.w = 65 / CORR_PIC_H_W; // on redimensionne la longueur premier pic
   placer_rects_pic (&var -> p_lvl [l].pics [6]); // on place correctement chacun des rects du pic

   var -> p_lvl [l].pics [8].Rect.x = LONG_WIND + 2575; // on place le premier pic en x
   ...
   var -> p_lvl [l].pics [9].Rect.y = var -> p_lvl [l].sol_sec [7].y - HAUT_PICS_NORM * 2; // on place le premier pic en ordonnées
   ...
   var -> p_lvl [l].pics [12].Rect.y = var -> p_lvl [l].sol_sec [7].y - HAUT_PICS_NORM * 2; // on place le premier pic en ordonnées
   ...
   placer_rects_pic (&var -> p_lvl [l].pics [12]); // on place correctement chacun des rects du pic
   ...
   var -> p_lvl [l].pics [13].Rect.y = var -> p_lvl [l].sol_sec [7].y - HAUT_PICS_NORM * 2; // on place le premier pic en ordonnées
   ...
   placer_rects_pic (&var -> p_lvl [l].pics [13]); // on place correctement chacun des rects du pic
   var -> p_lvl [l].pics [12].text_pic = gal -> pic; // on redimensionne la longueur pic
   ...
   for (i = 2 ; i < NB_CANNONS ; i += 1) {
      ...
      /* on positionne l'arrivée */
   var -> arrivee.rect.x = LONG_WIND + 7500; // on positionne le sprite arrivee en x
   var -> arrivee.rect.h = 125; // on lui donne sa hauteur en h
   var -> arrivee.rect.y = var -> p_lvl [l].Sol.y - 15 - var -> arrivee.rect.h; // on positionne le sprite arrivee en y
   var -> arrivee.rect.w = 130 / (180 / 125); // on lui donne sa longueur correspondante au h sans la déformer (en réalité, l'image fait 180 par 130p)
